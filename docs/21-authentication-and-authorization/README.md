# Розділ 21: Автентифікація та Авторизація (Вичерпний посібник)
...
Це ядро клієнтської частини системи. Після рефакторингу 23.01.2026 його відповідальність була значно спрощена.

**Файл:** `src/shared/lib/context/auth-context.tsx`

**Ключові аспекти:**

1.  **Єдина відповідальність:** `AuthProvider` тепер відповідає **виключно** за керування сесією користувача (`user`) та глобальним станом завантаження (`loading`).

2.  **Безпечна ініціалізація (`initializeAuth`):**
    -   При першому завантаженні провайдер викликає `await supabase.auth.getUser()`.
    -   `getUser()` завжди робить запит до сервера Supabase, щоб отримати гарантовано свіжі та автентифіковані дані користувача.
    -   **Важливо:** Логіка завантаження пов'язаних даних (робочий простір, роль, підписка) була **повністю видалена** з `AuthProvider`. Це було зроблено, оскільки попередня реалізація базувалася на хибному припущенні про існування єдиного "активного" воркспейса, що призводило до багів.

3.  **Підписка на зміни (`onAuthStateChange`):**
    -   Провайдер підписується на події сесії.
    -   **`SIGNED_IN` / `TOKEN_REFRESHED` / `USER_UPDATED`:** Оновлює об'єкт `user` з отриманої сесії.
    -   **`SIGNED_OUT`:** Повністю очищує дані користувача (`clearUser`), миттєво оновлюючи UI.

**Нова архітектура передбачає, що дані, специфічні для воркспейса (ролі, підписки, квоти), мають завантажуватися локально на відповідних сторінках (наприклад, в layout для `/dashboard/[slug]`).**
...
Компоненти отримують доступ до глобальних даних сесії через спеціалізовані хуки, що абстрагують `useAuthContext`.

**Файл:** `src/shared/hooks/use-auth.ts`

Після рефакторингу в цьому файлі залишилися тільки хуки, що відповідають за глобальний стан користувача:

-   **`useUser()`:** Повертає об'єкт `user` від Supabase та стан завантаження `{ user, loading }`.
-   **`useAuthenticated()`:** Повертає `boolean`. Найпростіший спосіб перевірити, чи увійшов користувач.

**Важлива примітка про архітектуру:**

Хуки `useWorkspace()`, `useSubscription()`, `useQuotas()`, `usePermission()` та інші, що залежали від даних конкретного воркспейса, **були видалені з глобального контексту**.

Причина в тому, що ці дані не є глобальними, а залежать від того, в якому воркспейсі користувач працює в даний момент (що визначається, наприклад, через URL `/dashboard/[slug]`).

**Новий підхід:**
-   Дані про воркспейс, ролі, підписки та квоти завантажуються на рівні відповідних сторінок або леяутів.
-   При необхідності, для передачі цих даних дочірнім компонентам, створюється **локальний контекст**, який охоплює тільки потрібну частину дерева компонентів.
