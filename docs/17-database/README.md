...
| `tasks` | Завдання для менеджерів. | `title` (назва), `due_date` (термін виконання), `assigned_to` (виконавець), `result` (результат). |
...
| Таблиця | Призначення | Ключові поля |
| :--- | :--- | :--- |
| `activities` | **Стрічка активності.** Зберігає всю історію дій в системі. | `activity_type`, `content` (опис дії), `user_id` (хто зробив), `contact_id`, `deal_id`, `task_id`. |
...
*   `update_workspace_invitations_updated_at()`: Спеціалізована версія попереднього тригеру для таблиці запрошень.
...
Допоміжні та викликані через API функції:
*   `get_current_workspace_id()` та `get_current_user_role()` використовуються в політиках безпеки для отримання контексту поточного користувача.
*   `generate_invitation_token()`: Використовується для генерації криптографічно стійкого токена при створенні нового запрошення в систему.
...
*   **Захист від небезпечних операцій:** Наприклад, видалити користувача з організації (`workspace_users`) може тільки `admin` або `owner`. Для таблиці `workspace_invitations` також налаштовані політики, які дозволяють адміністраторам керувати запрошеннями лише в межах своєї організації. Окремо існує політика для `workspace_quotas`, яка дозволяє адміністраторам оновлювати максимальні ліміти, що необхідно для роботи сервісу білінгу при зміні тарифу.
...
*   **Унікальний частковий індекс (`unique_pending_invitation`):** Спеціальний індекс для таблиці `workspace_invitations`. Він гарантує, що не можна надіслати два активних запрошення на одну й ту саму електронну адресу в межах однієї організації, що запобігає створенню дублікатів.
